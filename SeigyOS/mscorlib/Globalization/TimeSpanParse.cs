    ...
    internal static class TimeSpanParse
    {
        internal static void ValidateStyles(TimeSpanStyles style, string parameterName)
        {
            if (style != TimeSpanStyles.None && style != TimeSpanStyles.AssumeNegative)
                throw new ArgumentException(__Resources.GetResourceString(__Resources.Argument_InvalidTimeSpanStyles), parameterName);
        }
            ...
            out long result)
        {
            ...
            long ticks = ((long)days.num * 3600 * 24 + (long)hours.num * 3600 + (long)minutes.num * 60 + seconds.num) * 1000;
        ...
        // 
        private static bool TryParseExactTimeSpan(string input, string format, IFormatProvider formatProvider, TimeSpanStyles styles,
                ...
                switch (ch)
                {
                    ...
                    case 'd':
                        ...
                        if (tokenLen > 8 || seenDD ||
                            !ParseExactDigits(ref tokenizer, (tokenLen < 2) ? 1 : tokenLen, (tokenLen < 2) ? 8 : tokenLen, out tmp, out dd))
                        {
                            result.SetFailure(ParseFailureKind.Format, "Format_InvalidString");
                            return false;
                        }
        ...
        // 
        private static bool TryParseExactMultipleTimeSpan(string input, string[] formats, IFormatProvider formatProvider, TimeSpanStyles styles,
            ref TimeSpanResult result)
        {
            if (input == null)
            {
